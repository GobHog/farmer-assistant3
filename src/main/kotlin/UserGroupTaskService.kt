...
class UserGroupTaskService(database: Database) {
    object UserGroupTasks : Table("\"User_Group_task\"") {
        ...
        val user_id = reference("User_ID", UserService.Users.id)
        val group_task_id = reference("Group_task_ID", GroupTaskService.GroupTasks.id)
            ...
            UserGroupTasks.selectAll()
                ...
                .map {
                    ExposedUserGroupTask(
                        ...
                        user_id = it[UserGroupTasks.user_id].value,
                        ...
                        group_task_id = it[UserGroupTasks.group_task_id].value
                    )
                }
                .singleOrNull()
        }
    }
    suspend fun deleteUserGroupTaskById(id: Long) {
        dbQuery {
            UserGroupTasks.deleteWhere { group_task_id eq id }
        }
    }
    suspend fun deleteAllTasksByUserId(userId: Long) {
        dbQuery {
            UserGroupTasks.deleteWhere { user_id eq userId }
        }
    }
